(function(module) {
    ...
    var startTimer = function() {
        setInterval(function() {
            ProcessCache.forEach(function(process, index) {
               if (!process.startTime || new Date() - new Date(process.startTime) >= 0) {
                   process.onStart && process.onStart();
                   Processes.push(process);
                   ProcessCache.splice(index, 1);
               }
            });
            Processes.forEach(function(process, index) {
               if (process.endTime && new Date() - new Date(process.endTime) > 0) {
                   process.onEnd && process.onEnd();
